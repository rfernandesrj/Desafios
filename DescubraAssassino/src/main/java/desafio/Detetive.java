...
public class Detetive {
	...
	public Detetive() {
		this.geradorAleatorio = new GeradorAleatorioJavaRand();
	}
	...
	public int informarTeoria(int suspeito, int local, int arma) {
		...
		if (!existeRespostaArmazenada()){
			throw new IllegalStateException("Nenhuma resposta foi gerada");
		}
		...
		List<Integer> respostasErradas = new ArrayList<Integer>();
	...
	public int[] investiga() {
		...
		System.out.println(" ## Iteracoes a investigar: " + iteracoes);
